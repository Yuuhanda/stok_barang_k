...
if (empty($serial_n)) {
    // Get the SKU for the current item
    ...
    do {
        $autogenerated_sn = $sku . rand(1, 1000000);
        // Check if the generated serial number already exists in the database
        $sn_check = $mysqli->query("SELECT serial_number FROM barang_unit WHERE serial_number = '$autogenerated_sn'");
    } while ($sn_check->num_rows > 0); // Repeat until the serial number is unique

    // Insert the new unit with the unique serial number
    if ($mysqli->query("INSERT INTO barang_unit (id_barang, status, id_employee, id_gudang, id_user, comment, serial_number, kondisi) 
        VALUES ('$id_barang', '0', NULL, '$id_gudang', '$id_admin', 'Unit Baru', '$autogenerated_sn', '0')")) {
        header('Location: ../admin/item-detail.php?id=' . $id_barang);
        exit();
    } else {
