...
if ($status_val==0) {
    $temp_aval = $aval_stat + 1;
    $mysqli->query("UPDATE barang_unit SET status = '$status_val', id_employee = NULL ,id_gudang='$id_gudang',id_user ='$id_admin', comment ='$comment_content' WHERE id_unit = '$id_unit'");
    header('Location: ../admin/item-detail.php?id=' . $id_barang);   
    exit();
} elseif($status_val==1){
    $mysqli->query("UPDATE barang_unit SET status = '$status_val',id_employee='$id_employee',id_gudang=NULL ,id_user='$id_admin',comment='$comment_content' WHERE id_unit = '$id_unit'");
    header('Location: ../admin/item-detail.php?id=' . $id_barang);   
    exit();
} elseif($status_val==2){
    $mysqli->query("UPDATE barang_unit SET status ='$status_val',id_employee= NULL ,id_gudang= NULL ,id_user='$id_admin',comment='$comment_content' WHERE id_unit = '$id_unit'");
    header('Location: ../admin/item-detail.php?id=' . $id_barang);   
    exit();
} elseif($status_val==3){
    $mysqli->query("UPDATE barang_unit SET status ='$status_val',id_employee= NULL, id_gudang= NULL,id_user='$id_admin',comment='$comment_content' WHERE id_unit = '$id_unit'");
